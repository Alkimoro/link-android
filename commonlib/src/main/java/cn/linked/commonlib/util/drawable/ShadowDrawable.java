...
public class ShadowDrawable extends Drawable {
    ...
    private long backgroundSwitchAniStartTime = 0;
    private int backgroundSwitchDuration = 0;
    ...
    private void drawBackground(Canvas canvas) {
        if(subBackground == null) { // 没有在进行切换动画
            if(background != null) {
            ...
            long startTime = backgroundSwitchAniStartTime;
            long duration = backgroundSwitchDuration;
            if(currentTime - startTime <= duration) {
                float value = (currentTime - startTime)/(duration * 1.0f) * 255f;
                if(subBackground != null) {
                    subBackground.setBounds(content);
                    subBackground.draw(canvas);
                }
                if(background != null) {
    ...
    public void setBackground(Drawable background) {
        if (this.background != background) {
            if(backgroundSwitchDuration > 0) {
                backgroundSwitchAniStartTime = SystemClock.uptimeMillis();
                subBackground = this.background;
    ...
    public void setBackgroundSwitchDuration(int duration) {
        if (duration >= 0) {
            this.backgroundSwitchDuration = duration;
        }
    }
